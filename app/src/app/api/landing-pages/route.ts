...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const { name, pagePath, ga4PropertyId, sheetId, sheetName, sheetSourceFilter, sheetSourceColumn } = parsed.data;

  try {
    const { rows } = await db.query<{ id: string }>(
      `INSERT INTO landing_page (name, page_path, ga4_property_id, sheet_id, sheet_name, sheet_source_filter, sheet_source_column)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING id`,
      [name, pagePath, ga4PropertyId, sheetId ?? null, sheetName ?? null, sheetSourceFilter ?? null, sheetSourceColumn ?? null]
    );
